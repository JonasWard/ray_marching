...
const vec4 b_pln = vec4(.0, 0., 1., 19.1);
...
const vec3 circle = vec3(0., d2mid, sqrt( d2mid * d2mid + recSpacing * recSpacing * .25));
...
float sdWaveX(vec3 inputP) {
    ...
    return multiplier * sdCircle(temp) * cell.y;
}
