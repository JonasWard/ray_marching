...
vec3 recMod(vec3 p, float scale) {
    return mod(p, vec3(scale) ) - vec3( .5 * scale);
}

vec3 recModZScales(vec3 p, float scale) {
    vec3 pUpdate = vec3(p.xy, p.z / scale);
    return mod(pUpdate, vec3(scale) ) - vec3( .5 * scale);
}

vec2 recMod(vec2 p) {
    return mod(p, recVec) - halfRec;
}

vec2 recModB(vec2 p) {
    return mod(p - halfRec, recVec) - halfRec;
}

float voxelDistance(vec2 p) {
    return 0.;
}

float sdBullets(vec3 p){
    float val = mod(p.y, DOUBLEBH);
    val -= BANDHEIGHT;
    val = sqrt(SQUAREBH - val * val);
    return -val;
